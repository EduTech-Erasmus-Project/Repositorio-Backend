            ...
            for evaluationConceptQualification in evaluationConceptQualificationList:
                if (evaluationConceptQualification.evaluation_concept_id == evaluation_question.evaluation_concept_id):
                    evaluationQuestionsQualification = EvaluationQuestionsQualification.objects.create(
                        ...
                        qualification=self.create_evaluation_order_qualification(evaluation_question.id,
                                                                                 results_evaluation)
                    )
                    if (qualification != -1):
                        multiplicacion = int(qualification) * evaluation_question.weight
    ...
    def update(self, request, pk=None, project_pk=None):
        ...
        for evaluationQuestionsQualification, qualification in zip(evaluationQuestionsQualifications, scores):
            ...
            if int(qualification) != -1:
                ...
                multiplicacion = int(qualification) * wieght_question.weight
...
def updateAverage(scoreData: list, instances: list):
    ...
    for concept in scoreData:
        if (str(concept.concept_evaluations.evaluation_concept)) == concepts[0]:
            ...
            if int(concept.qualification) != -1:
                weight_question = EvaluationQuestion.objects.get(pk=concept.evaluation_question_id)
                multiplicacion = int(concept.qualification) * weight_question.weight
...
class EvaluationSelfQuestionSCHEMAViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None, project_pk=None):
        """
        Funcion para actualizar las preguntas de evaluacion
        """
        queryset = SelfEvaluationQuestions.objects.all()
        instance = get_object_or_404(queryset, pk=pk)
        serializer = EvaluationSelfQuestionSerializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        instance.description = serializer['description'].value
        instance.descriptionEnglish = serializer['descriptionEnglish'].value
        instance.save()
...
class RelationshipBetweenMetadataQuestion(RetrieveUpdateDestroyAPIView):
    """
        Servicio para actualizar las relaciones entre las preguntas y
        los metadatos ne la tabla de Schema, de la misma manera servicio para
        eliminar las relaciones de los metadatos con las preguntas
    """
    permission_classes = [IsAuthenticated, IsAdministratorUser]
...
class CreateMetadataAssessmentSelfQuestion(ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        """
            Funcion para guardar las preguntas que registra el usuario
            al momento de subir el objeto de aprendizaje
        """
        try:
...
def convertResponseEvalution(answer):
    """
        Funcion parar convertir las variables en los puntajes adecuados
    """
    if answer == 'yes':
        return 1
    if answer == 'no':
